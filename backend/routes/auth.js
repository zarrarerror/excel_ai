...
router.get('/me', async (req, res) => {
  ...
  if (isPro) {
    ...
    return res.json({ email: user.email, id: user.id, is_pro: true, lifetime_usage: used, monthly_usage: mu, monthly_limit: proLimit, remaining: Math.max(0, proLimit - mu), resets_at: nr.toISOString(), checkout_url: process.env.LEMONSQUEEZY_CHECKOUT_URL });
  }
